...
//            cout <<"imagbatch "<< imageBatch.scene_id << " image size "<< imageBatch.images_vec.size()<<endl;
    ...
    vector<GpsImageBatch> getAllGpsImageBatch()
    {
        vector<string> gpsFileNames; //GPS文件名
        vector<string> imagesFileNames;//所有image文件名
...
//                读每一帧的gps点
                ...
                if(gpsInfoEach.gpsPoints.size() == image_vec.size())
                {
                    for (int j = 1; j < gpsInfoEach.gpsPoints.size(); ++j) {
                        ...
                        cout << "-----image id------" << image_vec[j]<<endl;
                    }
                }else{
...
//                cout << "-----image_vec:------" << image_vec.size()<<endl;
//                imageBatch.images_vec = image_vec;
                dataFiles.push_back(gpsImageBatch);
            }
//            cout << "dfgfb:"<< dataFiles.size()<<" dfgfd:"<< dataFiles[100].images_vec.size()<< endl;
            return dataFiles;
        } else{
    ...
    GpsImageBatch getGpsImageBatchByImageId(string scene_id2, int index)
    {
        vector<string> gpsFileNames; //GPS文件名
        vector<string> imagesFileNames;//所有image文件名
        //    1.读gps, 2读image
