...
Description: 
This is to define a class to be constructed from:
...
class DepthMapping {
private:
    vector<PointInt> points_target;               //target region points
    Mat ref;                                         //ref image
    SE3 pose_ref_TWC;                                //ref_pose
    vector<Mat> currs;                               //curr images
    vector<SE3> poses_curr_TWC;                  //curr poses

public:
    // CONSTRUCTOR
    DepthMapping(Mat& ref, SE3& pose_ref, vector<Mat>& currs, vector<SE3>& poses_curr, vector<PointInt>& points) {
        this->ref = ref;
    ...
    bool update(const Mat& ref, const Mat& curr, const SE3& T_C_R, Mat& depth, Mat& depth_cov ){
        ...
        int total = points_target.size();

        for (int i = 0; i < points_target.size(); i++) {
            PointInt p = points_target[i];
            int x = p.x;
            int y = image_height - p.y;
            // 遍历每个像素
    ...
    inline bool inside( const Vector2d& pt ) {
        return pt(0,0) >= 0 && pt(1,0) >= 0
            && pt(0,0) < image_width && pt(1,0) <= image_height;
    }
};
